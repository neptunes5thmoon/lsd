...
read_shape = Coordinate((268-2*12, 268, 268)) * voxel_size
write_shape = Coordinate((164-2*12, 164, 164)) * output_voxel_size
context = (read_shape - write_shape) / 2

output_channels = 10
block_shape = np.array((164-2*12, 164, 164, output_channels))
model = None
...
def process_lsd(
    ...
    print("in: ", chunk.shape)
    inputs = [rescale_data(chunk, 158, 233)] #-456, -7168)] 
...
def process_chunk(idi: ImageDataInterface, input_roi: Roi):
    print(f"{input_roi=} pre grow")
    input_roi = input_roi.grow(context, context)
    print(f"{input_roi=} post grow")
