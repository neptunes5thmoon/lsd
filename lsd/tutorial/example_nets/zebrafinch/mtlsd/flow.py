...
output_voxel_size = Coordinate((20, 8, 8))
voxel_size = Coordinate((20, 8, 8))
input_voxel_size = Coordinate((20, 8, 8))

read_shape = Coordinate((96, 484, 484)) * voxel_size
write_shape = Coordinate((60, 272, 272)) * output_voxel_size
context = (read_shape - write_shape) / 2

output_channels = 10
block_shape = np.array((30, 110, 110, output_channels))
model = None

# setup_dir = "/nrs/cellmap/ackermand/downsample_larissas_networks/lsd"
setup_dir = "/nrs/saalfeld/heinrichl/fly_organelles/lsd/networks/zebrafinch/mtlsd"
checkpoint_lsd = "train_net_checkpoint_400000"
input_tensor_lsd = ["raw"]
output_tensor_lsd = ["lsds", "affs"]
...
def process_lsd(
    ...
    # output_data = output_data.transpose((1, 2, 3, 0))
