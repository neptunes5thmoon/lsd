...
def spawn_worker(
    ...
    def run_worker():
        ...
        process_cmd = [
            "python",
            "/groups/saalfeld/home/heinrichl/dev/fly-organelles-lsd/lsd/lsd/tutorial/example_nets/zebrafinch/mtlsd/inference.py",
            "start-worker",
            "-setup",
            f"{setup_dir}",
            "-ckpt",
...
@click.option("-input_tensor", type=str)
@click.option("-output_tensor", type=str, multiple=True)
@click.option("-no", "--num_outputs", type=int)
@click.option("-cs", "--channels", type=str, multiple=True)  # 0:ecs,1:plasma_membrane,2:ecs (comma seperated number:dataset_name)
...
def predict(
    ...
    for channel_split in channels:
        parsed_channels.append(channel.split(":") for channel in channel_split.split(","))
    ...
    read_shape = daisy.Coordinate(net_config['input_shape']) * raw.voxel_size
...
def start_worker(
    ...
    with open(os.path.join(setup_dir, 'config.json'), 'r') as f:
        ...
        input_tensorname = net_config[input_tensor]
    ...
    parsed_channels = []
    for channel_split in channels:
        parsed_channels.append(list(channel.split(":") for channel in channel_split.split(",")))
    ...
    mask_datasets = [open_ds(os.path.join(mc, md)) for mc, md in zip(mask_container, mask_dataset)]
